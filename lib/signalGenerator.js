...
const COINGECKO_API_ENV = String(process.env.COINGECKO_API_ENV || '').toLowerCase();
...
function getCoinGeckoClient() {
  ...
  const preferredEnv = COINGECKO_API_ENV === 'pro' ? 'pro' : 'demo';
  const sharedKey = COINGECKO_API_KEY;
  const proKey = COINGECKO_PRO_API_KEY || (preferredEnv === 'pro' ? sharedKey : '');
  const demoKey = COINGECKO_DEMO_API_KEY || (preferredEnv === 'demo' ? sharedKey : '');
...
async function fetchFromCoinGecko(symbol, timeframe, limit = 120, geckoIdOverride = null) {
  ...
  const data = await client.coins.marketChart.get(geckoId, {
    vs_currency: 'usd',
    days: String(days),
    interval,
  });
...
async function fetchCoinGeckoCoinContext(symbol, geckoIdOverride = null) {
  const client = getCoinGeckoClient();
  const geckoId = geckoIdOverride || SYMBOL_MAP[symbol]?.geckoId || 'bitcoin';
  return client.coins.getID(geckoId, {
    localization: false,
    tickers: false,
    market_data: true,
...
async function searchCoins(query, limit = 10) {
  ...
  try {
    const data = await client.search.get({ query: q });
...
async function fetchCatalystWatch(symbol, options = {}) {
  ...
  try {
    const data = await client.search.trending.get();
  ...
  try {
    const payload = await client.global.get();
